...
export const estimateFare = async (req, res) => {
  try {
    ...
    console.error("estimateFare error:", e.message);
...
export const createRide = async (req, res) => {
  try {
    ...
    if (!cabType) {
      return res.status(400).json({ success: false, message: "Cab type is required." });
    }

    // Get route (with fallback if API fails)
    const route = await getRoute(
    ...
    const ride = await Ride.create({
      ...
      pickup: {
        ...
        coordinates: { lat: parseFloat(pickupLat), lng: parseFloat(pickupLng) },
      },
      drop: {
        address: dropAddress,
        coordinates: { lat: parseFloat(dropLat), lng: parseFloat(dropLng) },
      },
    ...
    // Notify ALL online drivers via socket (use "newRideAvailable" to match driver listener)
  ...
  try {
    ...
    if (ride.genderPreference !== "ANY") {
      ...
      if (ride.genderPreference === "MALE_ONLY" && driverUser?.gender !== "MALE") {
        return res.status(403).json({ success: false, message: "This ride requires a male driver." });
      }
      if (ride.genderPreference === "FEMALE_ONLY" && driverUser?.gender !== "FEMALE") {
        return res.status(403).json({ success: false, message: "This ride requires a female driver." });
      }
    }
    ...
    // Notify riders in this ride room
    ...
    // Broadcast to all that this ride is no longer available
    req.io?.emit("rideUnavailable", { rideId: ride._id });

    res.json({ success: true, ride });
  } catch (e) {
    console.error("acceptRide error:", e.message);
    res.status(500).json({ success: false, message: e.message });
  }
};
...
export const cancelRide = async (req, res) => {
  try {
    ...
    const driver = req.user.role === "DRIVER"
      ? await Driver.findOne({ userId: req.user._id })
      : null;
    ...
    if (isRider && ride.status === "ARRIVING") ride.cancellationFee = 30;
    await ride.save();
...
// ── Available shared rides (with matching) ────────────────
export const getSharedRides = async (req, res) => {
  try {
    ...
    if (genderPref && genderPref !== "ANY") {
      filter.genderPreference = { $in: [genderPref, "ANY"] };
    }

    let rides = await Ride.find(filter).populate("driverId").lean();

    rides = rides.filter((r) => {
      const active = r.riders.filter((x) =>
        ["PENDING", "ACCEPTED", "ONBOARD"].includes(x.status)
      ).length;
      return active < r.maxRiders;
    });
...
export const joinSharedRide = async (req, res) => {
  try {
    ...
    const activeRiders = ride.riders.filter((r) =>
      ["PENDING", "ACCEPTED", "ONBOARD"].includes(r.status)
    );
    ...
    if (ride.genderPreference === "MALE_ONLY" && req.user.gender !== "MALE") {
      return res.status(403).json({ success: false, message: "Male passengers only." });
    }
    if (ride.genderPreference === "FEMALE_ONLY" && req.user.gender !== "FEMALE") {
      return res.status(403).json({ success: false, message: "Female passengers only." });
    }
...
export const getRideById = async (req, res) => {
  try {
    const ride = await Ride.findById(req.params.id).populate({
      path: "driverId",
      populate: { path: "userId", select: "name gender rating phone" },
    });
    if (!ride) return res.status(404).json({ success: false, message: "Ride not found." });
    res.json({ success: true, ride });
  } catch (e) {
